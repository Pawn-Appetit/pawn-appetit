...
export function createDurationFormatter(i18n: {
    ...
    return (value: unknown, lng?: string) => {
        ...
        if (hoursRemainder > 0)
            parts.push(
                `${hoursRemainder}${i18n.t("units.durationLabels.hours", { lng: currentLng })}`,
            );
        if (minutesRemainder > 0)
            parts.push(
                `${minutesRemainder}${i18n.t("units.durationLabels.minutes", { lng: currentLng })}`,
            );
        if (secondsRemainder > 0)
            parts.push(
                `${secondsRemainder}${i18n.t("units.durationLabels.seconds", { lng: currentLng })}`,
            );
...
export function createDurationLongFormatter(i18n: {
    ...
    return (value: unknown, lng?: string) => {
        ...
        if (hoursRemainder > 0)
            parts.push(
                `${hoursRemainder} ${i18n.t("units.durationLabels.hoursLong", { lng: currentLng })}`,
            );
        if (minutesRemainder > 0)
            parts.push(
                `${minutesRemainder} ${i18n.t("units.durationLabels.minutesLong", { lng: currentLng })}`,
            );
        if (secondsRemainder > 0)
            parts.push(
                `${secondsRemainder} ${i18n.t("units.durationLabels.secondsLong", { lng: currentLng })}`,
            );
        return (
            parts.join(", ") ||
            `0 ${i18n.t("units.durationLabels.secondsLong", { lng: currentLng })}`
        );
    };
}
 ...
 */
export function formatDateToPGN(
...
const TRANSLATED_PIECE_CHARS_CACHE = new Map<
    string,
    { K: string; Q: string; R: string; B: string; N: string }
>();
...
export function hasTranslatedPieceChars(
    ...
    return Object.keys(PIECE_SYMBOLS).some((key) => {
        const translatedChar = i18n.t(`chess.pieceChars.${key?.toLowerCase()}`, {
            lng: currentLng,
        });
...
export function createMoveNotationFormatter(
    ...
    return (
        ...
        const notationType =
            ...
            (storage
                ? getFromAtomWithStorage<"letters" | "symbols" | "letters-translated">(
                      storage,
                      "letters",
                      "symbols",
                  )
        ...
        switch (notationType) {
            ...
            case "letters-translated": {
                ...
                const pieceChar =
                    translatedPieceChars[move[0] as keyof typeof translatedPieceChars];
                if (typeof pieceChar === "undefined") return move;
                return pieceChar + move.slice(1);
            }
            default:
