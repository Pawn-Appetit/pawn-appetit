...
const ENGINES = [
    ...
    },
    {
        ...
        installMethod: "download" as const,
        downloadLink:
            "https://github.com/Matthies/RubiChess/releases/download/20240817/RubiChess-20240817.zip",
    ...
    },
    {
        ...
        installMethod: "download" as const,
        downloadLink:
            "https://pub-561e4f3376ea4e4eb2ffd01a876ba46e.r2.dev/lc0-v0.30.0-windows-gpu-nvidia-cuda.zip",
        ...
        queryFn: async () => {
            ...
            const supportedEngines = await Promise.all(
                availableEngines.map(async (engine) => {
                    const isSupported = await isInstallMethodSupported(
                        engine.installMethod || "download",
                    );
                    return isSupported ? engine : null;
                }),
            );

            return supportedEngines.filter(
                (engine): engine is NonNullable<typeof engine> => engine !== null,
            );
        },
        enabled: opened && !!os,
        staleTime: Infinity,
