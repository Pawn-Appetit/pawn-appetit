...
export class ProgressManager {
    ...
    calculateStats(practiceExerciseProgress: readonly PracticeExerciseProgress[]): ProgressStats {
    ...
    generateRecommendations(
        ...
        const completedExerciseIds = practiceExerciseProgress
    ...
    checkAchievements(
        ...
        if (
            currentStats.difficultyBreakdown.beginner.completed ===
                currentStats.difficultyBreakdown.beginner.total &&
        ...
        if (
            currentStats.totalPointsEarned >= 100 &&
            (previousStats?.totalPointsEarned ?? 0) < 100
        ) {
            newAchievements.push({
                id: "century",
                title: "Century Club",
    ...
    calculateAdaptiveDifficulty(
        ...
        const recentProgress = exerciseProgress
            .filter(
                (p) =>
                    p.lastAttempted &&
                    p.lastAttempted.getTime() > Date.now() - 7 * 24 * 60 * 60 * 1000,
            )
            .slice(-10);
        ...
        const avgAttempts =
            recentProgress.reduce((sum, p) => sum + p.attempts, 0) / recentProgress.length;
        const avgScore =
    ...
    exportProgress(practiceExerciseProgress: readonly PracticeExerciseProgress[]): string {
        const exportData = {
            version: "2.0",
    ...
    importProgress(exportedData: string): {
        ...
        try {
            ...
            throw new Error(
                `Failed to import progress data: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
        }
    }
}
