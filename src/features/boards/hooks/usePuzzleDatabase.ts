...
export const usePuzzleDatabase = () => {
    ...
    const loadPgnRatingRange = useCallback(
        async (dbPath: string) => {
            ...
            if (count > 0) {
                ...
                PUZZLE_DEBUG_LOGS &&
                    logger.debug("PGN rating bounds:", {
                        minRating,
                        maxRating,
    ...
    const createPuzzleFromGame = async (selectedGame: CachedPuzzle): Promise<Puzzle> => {
        ...
        if (parsedMoves.length !== moves.length) {
            logger.error(
                "Some moves could not be parsed from SAN to UCI. This needs to be fixed.",
                {
                    selectedGame,
                    parsedMoves,
                    moves,
    ...
    const generatePuzzle = async (
        ...
        if (dbInfo.path.endsWith(".db3")) {
            ...
            const dbPuzzle = await generatePuzzleFromPgn(
                db,
                currentRange[0],
                currentRange[1],
                !inOrder,
            ...
            if (!dbPuzzle) {
                throw new Error(
                    "Unable to generate a puzzle from local file within the requested range",
                );
            }
            PUZZLE_DEBUG_LOGS &&
