...
export const cleanupDuplicateThemesAtom = atom(null, (get, set) => {
    ...
    const cleanedThemes = customThemes.filter((theme) => {
        if (builtInThemeIds.has(theme.id)) {
            console.warn(
                `Removing custom theme "${theme.name}" (${theme.id}) as it conflicts with a built-in theme`,
            );
            return false;
        }
...
export const createThemeAtom = atom(
    null,
    (get, set, theme: Omit<Theme, "id" | "createdAt" | "updatedAt">) => {
        ...
        const existingTheme =
            customThemes.find((t) => t.id === newTheme.id) ||
            builtInThemes.find((t) => t.id === newTheme.id);
        if (existingTheme) {
...
export const updateThemeAtom = atom(
    null,
    (get, set, { id, updates }: { id: string; updates: Partial<Theme> }) => {
        const operations = get(themeOperationsAtom);
...
export const duplicateThemeAtom = atom(
    null,
    (get, set, { id, newName }: { id: string; newName?: string }) => {
        ...
        const existingTheme =
            customThemes.find((t) => t.id === duplicatedTheme.id) ||
            builtInThemes.find((t) => t.id === duplicatedTheme.id);
        if (existingTheme) {
...
export const importThemeAtom = atom(null, (get, set, themeData: ThemeExport) => {
    ...
    const existingTheme =
        customThemes.find((t) => t.id === importedTheme.id) ||
        builtInThemes.find((t) => t.id === importedTheme.id);
    if (existingTheme) {
